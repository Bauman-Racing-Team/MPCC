...
namespace mpcc
{
	static constexpr double VX_THRESHOLD = 3.0; // [m/s] vx threshold to calc combined (kinematic and dynamic) model derivatives
	...
	State Models::calculateSimpleCombinedModelDerivatives(const State &state, const Input &input) const
	{
		...
		double lambda = std::min(std::max((state(vxIdx) - VX_THRESHOLD) / 2., 0.), 1.);
	...
	std::pair<double, double> Models::calculateTireForces(double alpha, double kappa, double Fz, double Dfz, double fzNominal) const
	{
		...
		double Ex = (d_tire.PEX1 + d_tire.PEX2 * Dfz + d_tire.PEX3 * Dfz * Dfz) * (1. - d_tire.PEX4 * std::copysign(1.0, kappax)) * d_tire.LEX;

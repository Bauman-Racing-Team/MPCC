classdef Model
    ...
    function [Ffx,Ffy,Frx,Fry] = initSimpleFrictionEllipseConstraint(obj,states)
        ...
        function [Ffx,Ffy,Frx,Fry] = initCurvFrictionEllipseConstraint(obj, states)
                    vx = states(3);
                    vy = states(4);
                    r = states(5);
                    throttle = states(6);
                    steeringAngle = states(7);
                    brakes = states(8);
...
%                     Frrr = 200*tanh(vx);
        ...
        function rhs = initCurvilinearDynamicModel(obj, states, controls)
            curv=0.01;
                    n = states(1);
                    miu = states(2);
                    vx = states(3);
                    ...
                    sDot = (vx*cos(miu)-vy*sin(miu))/(1-n*curv);
                    rhsTemp = [vx*sin(miu)+vy*cos(miu);
                           r - curv * sDot;
                           1/m*...
                           (Frx+cos(steeringAngle)*Ffx+Fdrag-sin(steeringAngle)*Ffy +...
                           m*vy*r);
                           ...
                           (Fry+cos(steeringAngle)*Ffy+sin(steeringAngle)*Ffx-m*vx*r);
                           1/iz *...
                           (-Fry*lr+(cos(steeringAngle)*Ffy+sin(steeringAngle)*Ffx)*lf+Mtv);
                           dThrottle;
                           dSteeringAngle;
                           dBrakes];
                    rhs = rhsTemp./sDot;
